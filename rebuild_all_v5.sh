...
# Protobuf 3.12.4 uses CMakeLists.txt inside cmake/
cd cmake
...
# Force gRPC to use its own Abseil copy
mkdir -p build && cd build
...
find "$INFAAS_SRC" -name "*.pb.cc" -o -name "*.pb.h" | xargs -r rm -f  # -r prevents error if none found
...
cmake .. \
    ...
    -DProtobuf_LIBRARIES="$INSTALL_PREFIX/lib/libprotobuf.a" \
    -DgRPC_LIBRARIES="$INSTALL_PREFIX/lib/libgrpc++.a;$INSTALL_PREFIX/lib/libgrpc.a;$INSTALL_PREFIX/lib/libgpr.a" \
    -DProtobuf_INCLUDE_DIR="$INSTALL_PREFIX/include" \
    -DgRPC_INCLUDE_DIR="$INSTALL_PREFIX/include" \
