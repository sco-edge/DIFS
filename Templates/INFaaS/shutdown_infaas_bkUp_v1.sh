...
## PNB: Commented out the settings to require running on AWS or on the distributed service (2025/11/21)
...
## PNB: This is to allow running on only local server (2025.11.21)

echo "Shutting down local INFaaS..."

# Kill local processes
pkill -f modelreg_server
pkill -f queryfe_server
pkill -f master_vm_daemon
pkill -f worker_daemon
pkill -f redis-server

echo "Local INFaaS shut down."
