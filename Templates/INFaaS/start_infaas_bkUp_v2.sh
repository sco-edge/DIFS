#!/bin/bash
set +ex # PNB: I changed from '-ex' to '+ex' to not turn off error and to enable debugging whiles the code runs. I will turn it back of by switching back onces the code works
...
SECURITY_GROUP='sg-0c958981c725bb808'
IAM_ROLE='EC2-Role-for-INFaaSCodeTest'
MODELDB='infaas-s3-demo-bucket' # Model repository bucket (do not include s3://)
CONFIGDB='infaas-s3-demo-model-config-bucket' # Configuration bucket (do not include s3://)
WORKER_IMAGE='ami-0b8d527345fdace59'
