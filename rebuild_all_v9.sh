...
# THE ONLY FOUR LINES ADDED — THIS IS WHAT WAS MISSING
echo -e "${BLUE}==> Refreshing CMake cache to detect fixed linking rules...${NC}"
cmake .                                                                 # Re-run CMake to pick up worker-util linking fix
echo -e "${BLUE}==> Ensuring all required servers are built (worker_daemon is optional in local mode)...${NC}"
make -j$(nproc) modelreg_server queryfe_server || true
# worker_daemon/gpu_daemon only built if NVIDIA driver is present — ignore if missing

# FINAL SUCCESS VERIFICATION — LOCAL SINGLE-GPU MODE
BIN_DIR="$(pwd)/bin"
