...
class ModelExecutor(query_pb2_grpc.QueryServicer):
    ...
    def __init__(
        ...
        if os.path.exists(real_stats_path):
            ...
            self.mu_real = real_stats['mu']         # (2048, )
            self.sigma_real = real_stats['sigma']   # (2048, 2048)
            print(f"📊 실사 이미지의 mu 값 ({self.mu_real})")
            print(f"📊 실사 이미지의 sigma 값 계산 ({self.sigma_real})")
        else:
            print("[오류] Not Found: /workspace/real_stats.npz")
    ...
    async def QueryOnlineImage(self, request, context):
        ...
        if self.pipe is None:
            ...
            # 스트리밍이므로 yield로 응답 (단발성 오류 응답)
            yield query_pb2.QueryOnlineImageResponse(status=status_msg)
            return

        print(f"요청 수신(프롬프트: {request.Prompt} Steps: {request.Steps} Sampler_Type: {request.Sampler_Type}")
        ...
        try:
            ...
            chunk_size = 10  # 서버 메모리 보호를 위해 10개씩 분할
            ...
            self.fid.reset() # 새 요청마다 누적된 Fake 특징 초기화
            all_generated_tensors = []  # FID 계산용
            print(f"[서버] 총 {total_count}개 요청 수신. {chunk_size}개씩 분할 추론 시작...")
            ...
            for i in range(0, total_count, chunk_size):
                ...
                # 1. 추론 수행 (Mini-batch)
                with torch.inference_mode():
                    # autocast와 slicing을 활용하여 메모리 효율 극대화
                ...
                # 2. 결과 이미지 처리 및 개별 전송(Streaming)
                for j, pil_img in enumerate(output.images):
                    # (1) FID 계산을 위해 텐서로 변환하여 리스트에 보관
                    # [C, H, W] 형태의 uint8 텐서로 변환
                    img_tensor = torch.from_numpy(np.array(pil_img)).permute(2, 0, 1).unsqueeze(0).to(self.device)
                    #all_generated_tensors.append(img_tensor)

                    # FID 객체에 Fake 이미지로 업데이트 (특징 벡터만 저장됨)
                    self.fid.update(img_tensor, real=False)                    
                    # [메모리 핵심 2] 사용이 끝난 텐서는 즉시 메모리에서 삭제
                    del img_tensor

                    # (2) 이미지 바이너리화 및 클라이언트 전송
                    img_io = io.BytesIO()
                    pil_img.save(img_io, format="PNG")
                    img_bytes = img_io.getvalue()

                    # 출력 폴더 생성 및 이미지 저장 (generate_and_save_image 로직 적용)
                    target_dir.mkdir(parents=True, exist_ok=True)
                    print(f"생성된 이미지를 '{target_dir}' 폴더에 저장합니다.")

                    # 파일명 형식: sd_output_[샘플러명]_[인덱스]_[시드]_[해상도].png
                    filename = target_dir / f"sd_{sampler_name}_b{i+j+1}_s{request.Seed}_{width_px}x{height_px}.png"
                    pil_img.save(filename)
                    print(f"  -> 저장 완료: {filename}")
                    #saved_files.append(filename)

                    yield query_pb2.QueryOnlineImageResponse(
                        image_path=f"gen_{i+j}.png",
                        image_data=img_bytes,
                        status=infaas_request_status_pb2.InfaasRequestStatus(status=1)
                    )

            # 3. [핵심] 모든 루프 종료 후 100개 전체 FID 계산
            #if len(all_generated_tensors) >= total_count:
            print(f"[서버] {total_count}개 이미지 생성 완료. FID 계산 중...")

            # 4. 누적된 Fake 데이터로부터 mu/sigma 추출
            # fake_features_sum과 cov_sum을 이용하여 mu_fake, sigma_fake 계산
            num_samples = self.fid.fake_features_num_samples.item()
            mu_fake = (self.fid.fake_features_sum / num_samples).cpu().numpy()
            
            # 공분산 계산: (E[X^2] - E[X]^2)
            sigma_fake = (self.fid.fake_features_cov_sum.cpu().numpy() / num_samples) - np.outer(mu_fake, mu_fake)

            # 5. [중요] 미리 로드한 mu_real, sigma_real과 방금 구한 mu_fake, sigma_fake 비교
            final_fid = self.calculate_fid_from_stats(
                self.mu_real, self.sigma_real, 
                mu_fake, sigma_fake
            )
            
            print(f"🌟 [결과] {total_count}개 전체 FID 점수: {final_fid:.4f}")         

            """
            # 생성 이미지들을 하나의 배치로 묶음 [100, 3, H, W]
            generated_batch = torch.cat(all_generated_tensors, dim=0).to(self.device)
            
            # FID 계산 (self.fid 객체가 정의되어 있다고 가정)
            # real_features는 이미 self.fid.real_features 등에 로드되어 있어야 함
            self.fid.update(generated_batch, real=False)
            ...
            print(f"🌟 [결과] 100개 전체 FID: {final_fid:.4f}")
            """
        except Exception as e:
            print(f"[오류] 추론 중 발생: {str(e)}")
            context.set_details(str(e))
            context.set_code(grpc.StatusCode.INTERNAL)

    async def QueryOnlineImage_Old(self, request, context):
        """
        gRPC QueryOnline 요청 처리
        """
        if self.pipe is None:
        ...
        try:
            ...
            for path in saved_paths:
                ...
                # 성공 상태 메시지 (매 응답마다 포함하거나 마지막에만 포함하도록 proto 설계에 따라 조정)
                status_msg = infaas_request_status_pb2.InfaasRequestStatus(
                    status=infaas_request_status_pb2.SUCCESS
                )
                # [변경] return 대신 yield를 사용하여 이미지를 한 장씩 전송합니다.
                yield query_pb2.QueryOnlineImageResponse(
                    image_path=str(path),   # 현재 전송하는 이미지의 경로
                    image_data=image_binary,     # 현재 전송하는 이미지의 바이너리
                    status=status_msg
                )
                
                # [서버 안정성] 대량 전송 시 루프가 이벤트 루프를 독점하지 않도록 양보
                await asyncio.sleep(0.01)

            print(f"[정보] 총 {len(saved_paths)}개의 이미지 스트리밍 완료")
    ...
    def calculate_fid_from_stats(self, mu1, sigma1, mu2, sigma2):
        """
        Numpy 기반의 FID 계산 함수 (수치적 안정성 보정판)
        mu1, sigma1: 실제 이미지 통계 (Real)
        mu2, sigma2: 생성 이미지 통계 (Fake)
        eps: 수치적 안정성을 위한 아주 작은 상수
        """
        # 1. 입력 데이터를 numpy 배열로 확실히 변환 (1차원/2차원 보장)
        ...
        #from scipy import linalg
        # 2. 평균 간의 거리 (Squared Difference)
        diff = mu1 - mu2
       
        # 3. [핵심] 수치적 안정성을 위해 공분산 행렬에 Epsilon 대각선 추가
        # 행렬이 Singular(특이) 상태가 되는 것을 방지하여 sqrtm 오류를 막음
        offset = np.eye(sigma1.shape[0]) * eps
        
        # 4. 공분산 행렬의 기하평균 계산: sqrt(sigma1 * sigma2)
        # sigma1.dot(sigma2) 연산 시 발생할 수 있는 오류를 eps로 보정
        ...
        # 5. [중요] 연산 중 발생하는 미세한 복소수(허수) 제거
        # 수치 오차로 인해 아주 작은 허수 부분이 생길 수 있는데, 이를 실수로 변환
